...
public void insertNewStudent(int ID, int obor, String name, String surname, String date, float prumer) {
	if (date == null || name == null || surname == null)
	      throw new NullPointerException("Date, name and surname must be set.");

	    Connection conn = DBConnection.getDBConnection();

	    String insertUser = "INSERT INTO studenti" + "(ID, obor, name, surname, date, prumer)" + "VALUES(?,?,?,?,?,?)";

	    try (PreparedStatement prStmt = conn.prepareStatement(insertUser)) {
	      prStmt.setLong(1, ID);
	      prStmt.setLong(1, obor);
	      prStmt.setString(3, name);
	      prStmt.setString(4, surname);
	      prStmt.setString(5, date);
	      prStmt.setFloat(6, prumer);
